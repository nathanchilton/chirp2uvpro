...
def test_btech_parser_with_json_no_prefix():
    content = '{"chs":[{"n":"Test","tf":"146.520","rf":"146.520","ts":"13180","s":"1","p":"H","m":"FM"}]}'
    parser = BtechParser()
    channels = parser.parse(content)
    assert len(channels) == 1
    assert channels[0]['name'] == 'Test'
...
def test_btech_parser_with_json_no_prefix():
    content = '{"chs":[{"n":"Test","f":"14string","d":"0","t":"None","dt":"None","s":"0","m":"FM"}]}'
    # Note: the frequency in my test is '14string' which is invalid, 
    # but I want to test the parsing of the JSON structure itself.
    # Actually let's use a valid frequency.
